...
import { deleteCart, cartAdded, quantityDecreased, quantityIncreased } from "../../../features/cart/cartSlice"
...
const ListCart = () => {
    ...
    return (
        <Container fluid='true' style={{ margin: '30px' }} className="page-container">
            <h1>Shopping Cart</h1>
            <Row className="justify-content-center row-container">
                ...
                <Col lg={3} className="total-sidebar">
                    <h4>Total Items: {carts.reduce((total, cart) => total + (cart.quantity), 0)}</h4>
                    <br></br>
                    ...
                    <h5>Total Price: ${carts.reduce((total, cart) => total + (cart.price * cart.quantity), 0)}</h5>
                </Col>
            </Row>
